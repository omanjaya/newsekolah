  ...
  it.each(["#FFFFFF", "#FFFF00", "#6288BC", "#1F3A5F", "#000000", "#00FF00", "#F0F", "#8B4513"])(
    ...
    (hex) => {
      for (const [accent, surface] of [
        [lightVariantOf(hex), "#F7F6F3"],
        [darkVariantOf(hex), "#1C1C1C"],
      ] as const) {
...
describe("darkVariantOf", () => {
  it("lifts the default tenant accent off the dark surface", () => {
    // The bug this exists for: #1F3A5F is 1.48:1 on #1C1C1C, invisible.
