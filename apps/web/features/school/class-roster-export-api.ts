...
/** Either a single class or a whole grade level ("angkatan"), for the class roster export's scope picker. */
...
/**
 * Downloads the class roster export (XLSX or PDF, per options.format) for
 * one class or a whole grade level -- NIS, NISN, name, gender, birth
 * place/date, guardian name where recorded, one section per class.
 */
export async function downloadClassRosterExport(
  ...
  const response = await fetch(
    `${API_URL}/v1/academic/classes/roster/export?${params.toString()}`,
    {
  ...
  try {
    ...
    link.href = url;
    link.download = `daftar-siswa.${options.format}`;
