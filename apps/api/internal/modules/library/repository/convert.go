...
func toItemEvent(row db.LibraryItemEvent) domain.ItemEventRecord {
	return domain.ItemEventRecord{
		...
		MemberUserID: pdatabase.UUIDOrNil(row.MemberUserID), EventType: domain.ItemEvent(row.EventType), Notes: row.Notes,
		CreatedBy: pdatabase.UUIDOrNil(row.CreatedBy), CreatedAt: pdatabase.TimeOrZero(row.CreatedAt),
	}
}
