...
func (r *Repository) CreateItemEvent(ctx context.Context, e domain.ItemEventRecord) (domain.ItemEventRecord, error) {
	row, err := r.queries(ctx).CreateItemEvent(ctx, db.CreateItemEventParams{
		TenantID: e.TenantID, CopyID: e.CopyID, LoanID: pdatabase.NullUUID(e.LoanID), MemberUserID: pdatabase.NullUUID(e.MemberUserID),
		EventType: string(e.EventType), Notes: e.Notes, CreatedBy: pdatabase.NullUUID(e.CreatedBy),
	})
	if err != nil {
		return domain.ItemEventRecord{}, fmt.Errorf("create item event: %w", err)
	}
