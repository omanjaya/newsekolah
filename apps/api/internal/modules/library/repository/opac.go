...
func (r *Repository) SearchOpacTitles(ctx context.Context, tenantID uuid.UUID, p service.OpacSearchParams) ([]domain.Title, error) {
	...
	if len([]rune(p.Search)) >= 3 {
		rows, err := r.queries(ctx).SearchOpacTitlesFulltext(ctx, db.SearchOpacTitlesFulltextParams{
			TenantID: tenantID, ClassificationPrefix: pdatabase.Text(deref(prefix)), MaterialTypeID: pdatabase.NullUUID(p.MaterialTypeID), Search: p.Search,
	...
	params := db.SearchOpacTitlesShortParams{
		TenantID: tenantID, MaterialTypeID: pdatabase.NullUUID(p.MaterialTypeID),
		Limit: int32(p.Limit), Offset: int32(p.Offset), //nolint:gosec // clamped
	}
	if prefix != nil {
...
func (r *Repository) ListOpacMostBorrowedTitles(ctx context.Context, tenantID uuid.UUID, limit int) ([]domain.Title, error) {
	...
	for i, row := range rows {
		out[i] = toTitle(db.LibraryTitle{
			...
			Publisher: row.Publisher, PublishPlace: row.PublishPlace, PublishYear: row.PublishYear,
			...
			TargetAudience: row.TargetAudience, Notes: row.Notes, Abstract: row.Abstract, MaterialTypeID: row.MaterialTypeID,
