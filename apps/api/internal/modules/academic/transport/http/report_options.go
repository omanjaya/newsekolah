...
// et al.) into reportdoc.Options. Every parameter is optional; leaving
// all of them out (an unmigrated caller, including the mobile app)
// produces the zero Options that keeps a report's old default rendering:
// xlsx, every column, letterhead on. Mirrors attendance/scheduling/
// grading's identically named helpers (duplicated, not shared, per
// docs/03-layered-architecture.md section 1: transport packages own
// their own request parsing).
func reportdocOptions(format *api.ExportClassRosterParamsFormat, title *string, letterhead *bool, columns *string) reportdoc.Options {
	opts := reportdoc.Options{Format: reportdoc.FormatXLSX, ShowLetterhead: true}
	if format != nil && *format == api.ExportClassRosterParamsFormatPdf {
