...
func init() {
	for code, msg := range map[string][2]string{
		...
		"VIOLATION_INVALID":               {"Jenis pelanggaran tidak valid.", "The violation type is invalid."},
		"EVIDENCE_REQUIRED":               {"Bukti wajib dilampirkan.", "Evidence is required."},
		"HOMEROOM_TEACHER_REQUIRED":       {"Kelas ini belum memiliki wali kelas.", "This class has no homeroom teacher assigned."},
		"EXIT_PERMIT_ALREADY_TODAY":       {"Siswa sudah memiliki izin keluar hari ini.", "The student already has an exit permit for today."},
