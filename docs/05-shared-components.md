...
- `Document{Letterhead, Title, Scope []ScopeLine, Columns []Column, Sections []Section, Signature, PageLabelFormat, EmptyRowsLabel}`. Satu `Section` = satu sheet (XLSX) atau satu kelompok halaman (PDF) — laporan satu kelas punya satu `Section`, laporan satu angkatan punya satu `Section` per kelas. `Section` tanpa `Rows` merender satu baris `EmptyRowsLabel` (mis. "Tidak ada data") alih-alih header tanpa isi.
