...
| Kop surat bergambar pada laporan bulanan perpustakaan          | Baru nama sekolah dalam teks. Infrastrukturnya (kop laporan tenant bisa diisi di Pengaturan, dipakai laporan presensi pegawai/supervisi/tamu/sebagian besar laporan perpustakaan) sudah ada sejak sesi 24 September; laporan bulanan perpustakaan sendiri (`library/service/monthly_report.go`) belum disambungkan |
...
| Laporan ringkasan akreditasi perpustakaan belum di `reportdoc` | `getLibraryCatalogueSummaryReportXlsx` punya dua sheet dengan jumlah kolom berbeda (2 vs 3), tidak cocok dengan model satu set kolom per Document di `reportdoc`; perlu dipecah jadi dua laporan atau `reportdoc` diberi kolom per section                                                                         |
